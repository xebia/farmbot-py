...
class CeleryNode(object):
    ...
    def __str__(self):
        body = f", {str(self.body)}" if self.body else ""
        comment = f", {str(self.comment)}" if self.comment else ""
        return "{}('{}', {}{}{})".format(self.__class__.__name__, self.kind, self.args, body, comment)
...
class RPCRequest(CeleryNode):
    ...
    def __init__(self, command: CeleryNode):
        ...
        super().__init__('rpc_request', {'label': self.uuid})
